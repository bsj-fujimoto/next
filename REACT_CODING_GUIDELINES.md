...
#### 既存コンポーネントの確認と再利用
```tsx
// Good: 実装前に既存のコンポーネントを確認し、再利用可能なものは再利用する
// 例: DropdownMenuコンポーネントを作成し、ItemsPerPageDropdownやAvatarDropdownで共通利用
// 共通化できる機能は共通コンポーネントとして実装し、デザインの一貫性を保つ

// Bad: 既存のコンポーネントを確認せずに、同じ機能を重複実装する
// Bad: 既存コンポーネントと異なるデザインパターンを使用する（一貫性が損なわれる）
```

#### デザインの一貫性
```tsx
// Good: 既存コンポーネントと同じデザインパターンを使用する
// 例: 既存のDropdownMenuが白背景（bg-white）を使用している場合、新しいDropdownも同じデザインを使用

// Bad: 既存コンポーネントと異なるデザインパターンを使用する
// 例: 既存のDropdownMenuが白背景なのに、新しいDropdownが半透明（ガラスモーフィズム）を使用
```
